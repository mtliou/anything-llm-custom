...
2. After successful login, Run the following command to create a deployment using the Deployment Manager CLI:
  ...
  gcloud deployment-manager deployments create bridge-deployment --config gcp/deployment/gcp_deploy_bridge_with_env.yaml
...
```
gcloud compute instances get-serial-port-output bridge-instance 
```

ssh into the instance

```
gcloud compute ssh bridge-instance 
```

Delete the deployment
```
gcloud deployment-manager deployments delete bridge-deployment 
```
